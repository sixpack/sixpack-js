(function () {
    ...
    sixpack.Session.prototype = {
        participate: function(experiment_name, alternatives, force, callback) {
            ...
            if (!(/^[a-z0-9][a-z0-9\-_ ]*$/).test(experiment_name)) {
                return _handle_error(callback, new Error("Bad experiment_name"));
            }

            if (alternatives.length < 2) {
                return _handle_error(callback, new Error("Must specify at least 2 alternatives"));
            }

            for (var i = 0; i < alternatives.length; i += 1) {
                if (!(/^[a-z0-9][a-z0-9\-_ ]*$/).test(alternatives[i])) {
                    return _handle_error(callback, new Error("Bad alternative name: " + alternatives[i]));
                }
            }
    ...
    var _handle_error = function(callback, error) {
        if (on_node) {
